...
MODEL_NAME = "mse_lstm_seq2seq_additive_regu_31_07"  # mse_lstm_naive, mse_lstm_stacked, lstm_seq2seq_additive_corrected, mse_lstm_seq2seq_additive_regu_31_07
...
smard_production_df = pd.read_csv(
    rf"electricity_data/creation_forecast_day_ahead_{DATA_NAME}.csv", delimiter=";"
)
smard_production_df = smard_production_df.iloc[:, :3].copy()

smard_consumption_df = pd.read_csv(
    rf"electricity_data/consumption_forecast_{DATA_NAME}.csv", delimiter=";"
)
smard_consumption_df = smard_consumption_df.iloc[:, :3].copy()
...
print(f"smard consumption columns: {smard_consumption_df.columns}")
print(f"smard production columns: {smard_production_df.columns}")
...
##################################
# this is for smard columns conversion
smard_production_df.columns = ["start_time", "end_time", "smard_total_production_mwh"]
smard_consumption_df.columns = ["start_time", "end_time", "smard_total_load_mwh"]
...
# Convert numeric columns in smard production data
for col in smard_production_df.columns[2:]:
    smard_production_df[col] = smard_production_df[col].apply(utils.convert_to_float)

for col in smard_consumption_df.columns[2:]:
    smard_consumption_df[col] = smard_consumption_df[col].apply(utils.convert_to_float)
...
# smard production data
smard_production_df["start_time"] = pd.to_datetime(
    smard_production_df["start_time"], format="%d.%m.%Y %H:%M"
)
smard_production_df["end_time"] = pd.to_datetime(
    smard_production_df["end_time"], format="%d.%m.%Y %H:%M"
)

# smard consumption data
smard_consumption_df["start_time"] = pd.to_datetime(
    smard_consumption_df["start_time"], format="%d.%m.%Y %H:%M"
)
smard_consumption_df["end_time"] = pd.to_datetime(
    smard_consumption_df["end_time"], format="%d.%m.%Y %H:%M"
)
...
data = pd.concat(
    [production_df, consumption_df, smard_consumption_df, smard_production_df], axis=1
)
...
data.to_csv(rf"data/transformed/transformed_{MODEL_NAME}_{DATA_NAME}.csv", index=False)
...
# TODO: add the predicted one from smard
...
if PRETRAINED:
    ...
    smards_features = ["smard_" + feature for feature in output_features]
    # priliminary metrics
    metrics = evaluation.calculate_error_metrics(
        ...
        output_features=output_features,
        path="results",
        name=f"PRED_{MODEL_NAME}_{DATA_NAME}",
    )
    smart_metrics = evaluation.calculate_error_metrics(
        df=merged_df,
        prediction_features=smards_features,
        output_features=output_features,
        path="results",
        name=f"SMARD_{MODEL_NAME}_{DATA_NAME}",
    )
